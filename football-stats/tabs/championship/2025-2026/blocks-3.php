<?php
/**
 * Block 3: Mid-Table Security (Positions 7-14)
 * Safe from relegation, no promotion pressure
 */
...
$teams = array_values(array_filter($tableView['standings'], function ($team) {
	return $team['position'] >= 7 && $team['position'] <= 14;
}));
...
<div class="block-detail-page">
	...
	<div class="panel" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
		...
		<?php football_stats_render_table_view_controls($tableView, $currentMainTab ?? '2025-2026', $currentLeague ?? 'championship', $currentSubTab ?? 'blocks-3'); ?>
		<p style="margin-top: 15px; font-size: 1.1em; color: #ddd;">
			<?php echo $blockInfo['description']; ?> - The comfortable middle ground where teams are safe from relegation worries but unlikely to challenge for promotion.
		</p>
