...
foreach ($allTeams as $team) {
    ...
    if ($projectedPoints >= 85) $predictedBlock = 1; // Auto Promotion
    elseif ($projectedPoints >= 72) $predictedBlock = 2; // Playoff zone
    ...
    if ($team['position'] <= 3) $currentBlock = 1;
    elseif ($team['position'] <= 7) $currentBlock = 2;
    elseif ($team['position'] <= 19) $currentBlock = 3;
    elseif ($team['position'] <= 23) $currentBlock = 4;
    else $currentBlock = 5;

    // Final block based on current position
    if ($team['position'] <= 3) $finalBlock = 1;
    elseif ($team['position'] <= 7) $finalBlock = 2;
    elseif ($team['position'] <= 19) $finalBlock = 3;
    elseif ($team['position'] <= 23) $finalBlock = 4;
    else $finalBlock = 5;

    // Calculate various targets
    $pointsFor47 = max(0, 47 - $team['points']);
    $pointsFor85 = max(0, 85 - $team['points']);
    $ppgFor47 = $remainingGames > 0 ? round($pointsFor47 / $remainingGames, 2) : 0;
    $ppgFor85 = $remainingGames > 0 ? round($pointsFor85 / $remainingGames, 2) : 0;

    // Risk assessment
    if ($projectedPoints >= 47) {
        $risk = 'Safe';
        $riskColor = '#4CAF50';
    ...
    $projections[] = [
        ...
        'pointsFor47' => $pointsFor47,
        'pointsFor85' => $pointsFor85,
        'ppgFor47' => $ppgFor47,
        'ppgFor85' => $ppgFor85,
        'risk' => $risk,
        'riskColor' => $riskColor
...
<div class="dynamic-blocks-page">
    ...
    <div class="panel" style="border-left: 4px solid #00D9FF;">
        ...
        <?php football_stats_render_combined_table_controls($tableView, $currentMainTab ?? '2025-2026', 'league-two', $currentSubTab ?? 'blocks-dynamic'); ?>
    ...
    <div class="panel">
        ...
        <div class="methodology-grid">
            ...
            <div class="method-card">
                ...
                <p class="note">Extrapolates current form across the entire 46-game season</p>
            </div>
            <div class="method-card">
                <h4>3️⃣ Assign Predicted Block</h4>
                <p><strong>Based on Historical Ranges</strong></p>
                <p class="note">85+ pts = Block 1, 72+ = Block 2, 47+ = Block 3, 38+ = Block 4, &lt;38 = Block 5</p>
            </div>
            <div class="method-card">
                <h4>4️⃣ Risk Assessment</h4>
                <p><strong>Survival Probability</strong></p>
                <p class="note">Safe (47+), Caution (38-46), Danger (&lt;38) based on relegation history</p>
            </div>
        </div>
    </div>
        ...
        <div class="predicted-blocks-grid">
            ...
            <div class="predicted-block-card" style="border-left: 4px solid <?php echo $blockColors[$blockNum]; ?>;">
                ...
                <ul class="predicted-teams-list">
                    ...
                    <li>
                        <span class="team-name-plain"><?php echo htmlspecialchars($proj['team']['team_name']); ?></span>
        ...
        <div class="targets-grid">
            ...
            <div class="target-card" style="border-left: 3px solid <?php echo $proj['riskColor']; ?>;">
                ...
                <div class="target-stats">
                    ...
                    <?php if ($proj['projected'] < 85 && $proj['currentBlock'] <= 2): ?>
                    <div style="border-top: 1px solid rgba(255,255,255,0.1); margin: 10px 0; padding-top: 10px;">
                        <div class="target-row">
                            <span class="target-label">🏆 For 85pt Auto Promotion:</span>
                            <span class="target-value" style="color: #FFD700;">
                                Need <?php echo $proj['pointsFor85']; ?> pts
                            </span>
                        </div>
                        <div class="target-row">
                            <span class="target-label">Required PPG:</span>
                            <span class="target-value" style="color: #FFD700; font-weight: bold;">
                                <?php echo $proj['ppgFor85']; ?>
                            </span>
                        </div>
                    </div>
                    <?php endif; ?>
    ...
    <div class="panel">
        ...
        <div class="insights-dynamic-grid">
            <?php
            $safeTeams = count(array_filter($projections, fn($p) => $p['projected'] >= 47));
            ...
            <div class="insight-dynamic-card" style="border-left: 3px solid #4CAF50;">
                ...
                <p>Currently on track for 47+ points (safety threshold)</p>
            </div>
            ...
            <div class="insight-dynamic-card" style="border-left: 3px solid #5865F2;">
                ...
                <p>Average points per game across all 24 teams</p>
            </div>
        </div>
    </div>
    ...
    <div class="panel">
        <div class="block-navigation">
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-two', 'blocks-overview', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn">← Blocks Overview</a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-two', 'table', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn">Full Table</a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-two', 'blocks-1', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn">Detailed Blocks →</a>
        </div>
    </div>
</div>
