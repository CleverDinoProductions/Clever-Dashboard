<?php
// TEAM SURVIVAL TRACKER (League Two) - Data queries
...
if (!$team) {
    ...
    echo "<p>Please check that League Two data has been loaded.</p>";
...
// Survival status
if ($team['position'] <= 22 && $team['points'] >= $safety_target) {
    $survival_status = 'SAFE';
    $status_color = '#43b581';
    $status_icon = '✅';
} elseif ($team['position'] <= 22 && $halfway_progress_pct >= 75) {
...
<div class="panel" style="border: 3px solid <?= $teamSecondary ?>; background: linear-gradient(135deg, <?= $teamPrimary ?> 0%, #2e3136 100%);">
    <div style="text-align: center;">
        ...
        <?php football_stats_render_combined_table_controls($tableView, $currentMainTab ?? '2025-2026', $currentLeague ?? 'league-two', $currentSubTab ?? 'team-tracker'); ?>
    </div>
</div>
