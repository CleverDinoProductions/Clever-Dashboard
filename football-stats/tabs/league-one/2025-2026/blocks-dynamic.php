 ...
 * Real-time analysis with PPG-based season projections for League One
 */
...
foreach ($allTeams as $team) {
    ...
    // Determine predicted block based on projected points (League One thresholds)
    $predictedBlock = 5; // Default to worst case
    if ($projectedPoints >= 88) $predictedBlock = 1; // Auto promotion
    elseif ($projectedPoints >= 75) $predictedBlock = 2; // Playoff zone
    elseif ($projectedPoints >= 48) $predictedBlock = 3; // Mid-table
    elseif ($projectedPoints >= 40) $predictedBlock = 4; // Danger zone

    // Current block based on position
    if ($team['position'] <= 2) $currentBlock = 1;
    elseif ($team['position'] <= 6) $currentBlock = 2;
    elseif ($team['position'] <= 17) $currentBlock = 3;
    elseif ($team['position'] <= 21) $currentBlock = 4;
    else $currentBlock = 5;

    // Final block based on current position
    if ($team['position'] <= 2) $finalBlock = 1;
    elseif ($team['position'] <= 6) $finalBlock = 2;
    elseif ($team['position'] <= 17) $finalBlock = 3;
    elseif ($team['position'] <= 21) $finalBlock = 4;
    else $finalBlock = 5;

    // Calculate various targets
    $pointsFor40 = max(0, 40 - $team['points']);
    $pointsFor88 = max(0, 88 - $team['points']);
    $ppgFor40 = $remainingGames > 0 ? round($pointsFor40 / $remainingGames, 2) : 0;
    $ppgFor88 = $remainingGames > 0 ? round($pointsFor88 / $remainingGames, 2) : 0;

    // Risk assessment
    if ($projectedPoints >= 48) {
        $risk = 'Safe';
        $riskColor = '#4CAF50';
    } elseif ($projectedPoints >= 40) {
        $risk = 'Caution';
        $riskColor = '#FFA500';
    } else {
        $risk = 'Danger';
...
<div class="dynamic-blocks-page">
    ...
    <div class="panel" style="border-left: 4px solid #00D9FF;">
        ...
        <?php football_stats_render_combined_table_controls($tableView, $currentMainTab ?? '2025-2026', 'league-one', $currentSubTab ?? 'blocks-dynamic'); ?>
    ...
    <div class="panel">
        ...
        <div class="methodology-grid">
            ...
            <div class="method-card">
                ...
                <p class="note">Extrapolates current form across entire 46-game League One season</p>
            </div>
            <div class="method-card">
                <h4>3️⃣ Assign Predicted Block</h4>
                <p><strong>Based on Historical Ranges</strong></p>
                <p class="note">88+ pts = Block 1, 75+ = Block 2, 48+ = Block 3, 40+ = Block 4, &lt;40 = Block 5</p>
            </div>
            <div class="method-card">
                <h4>4️⃣ Risk Assessment</h4>
                <p><strong>Survival Probability</strong></p>
                <p class="note">Safe (48+), Caution (40-47), Danger (&lt;40) based on relegation history</p>
            </div>
        </div>
    </div>
        ...
        <div class="targets-grid">
            ...
            <div class="target-card" style="border-left: 3px solid <?php echo $proj['riskColor']; ?>;">
                ...
                <div class="target-stats">
                    ...
                    <?php if ($proj['projected'] < 88 && $proj['currentBlock'] <= 2): ?>
                    <div style="border-top: 1px solid rgba(255,255,255,0.1); margin: 10px 0; padding-top: 10px;">
                        <div class="target-row">
                            <span class="target-label">🏆 For 88pt Auto Promo:</span>
                            <span class="target-value" style="color: #FFD700;">
                                Need <?php echo $proj['pointsFor88']; ?> pts
                            </span>
                        </div>
                        <div class="target-row">
                            <span class="target-label">Required PPG:</span>
                            <span class="target-value" style="color: #FFD700; font-weight: bold;">
                                <?php echo $proj['ppgFor88']; ?>
                            </span>
                        </div>
                    </div>
                    <?php endif; ?>
    ...
    <div class="panel">
        ...
        <div class="insights-dynamic-grid">
            ...
            <div class="insight-dynamic-card" style="border-left: 3px solid #4CAF50;">
                ...
                <p>Currently on track for 48+ points (comfortable safety)</p>
            </div>
    ...
    <div class="panel">
        <div class="block-navigation">
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-overview', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn">← Blocks Overview</a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'table', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn">Full Table</a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-1', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn">Detailed Blocks →</a>
        </div>
    </div>
</div>
