<?php
/**
 * Blocks Overview - League One Blocks Framework
...
$blocks = [
    1 => ['teams' => [], 'range' => '1-2', 'title' => 'Auto Promotion', 'emoji' => '🏆', 'color' => '#FFD700', 'desc' => 'Automatic Promotion to Championship'],
    2 => ['teams' => [], 'range' => '3-6', 'title' => 'Playoff Contenders', 'emoji' => '🎯', 'color' => '#5865F2', 'desc' => 'Playoff spots for promotion to the Championship'],
    3 => ['teams' => [], 'range' => '7-17', 'title' => 'Mid-Table Security', 'emoji' => '⚖️', 'color' => '#99AAB5', 'desc' => 'Safe & Stable'],
    4 => ['teams' => [], 'range' => '18-21', 'title' => 'Relegation Battle', 'emoji' => '⚠️', 'color' => '#FFA500', 'desc' => 'Danger Zone'],
    5 => ['teams' => [], 'range' => '22-24', 'title' => 'Relegation Zone', 'emoji' => '🔴', 'color' => '#F44336', 'desc' => 'Drop Zone - League Two Bound']
];
...
<div class="blocks-overview-page">
    ...
    <div class="panel" style="border-left: 4px solid #5865F2;">
        ...
        <?php football_stats_render_combined_table_controls($tableView, $currentMainTab ?? '2025-2026', 'league-one', $currentSubTab ?? 'blocks-overview'); ?>
        <p style="margin-top: 15px; font-size: 1.1em; color: #ddd;">
            The League One is divided into 5 strategic blocks, each representing distinct competitive zones
    ...
    <div class="panel" style="border-left: 4px solid <?php echo $block['color']; ?>;">
        ...
        <div class="block-stats-summary">
            <?php
                ...
                $avgPPG = count($block['teams']) > 0 ? round($avgPoints / max($currentMatchday, 1), 2) : 0;
                $totalGD = array_sum(array_column($block['teams'], 'gd'));

                // Calculate block-specific targets for League One
                $blockTargets = [
                    1 => ['target' => '88-95 pts', 'ppg' => '2.2-2.5', 'status' => 'Automatic promotion'],
                    2 => ['target' => '75-88 pts', 'ppg' => '1.8-2.2', 'status' => 'Playoff qualification'],
                    3 => ['target' => '50-74 pts', 'ppg' => '1.1-1.7', 'status' => 'Safe mid-table'],
                    4 => ['target' => '40-49 pts', 'ppg' => '0.9-1.1', 'status' => 'Survival battle'],
                    5 => ['target' => '<40 pts', 'ppg' => '<0.9', 'status' => 'Relegation danger']
                ];
            ?>
        ...
        <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-' . $blockNum, $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="view-detail-btn" style="border-color: <?php echo $block['color']; ?>; color: <?php echo $block['color']; ?>;">
    ...
    <div class="panel">
        ...
        <div class="framework-grid">
            <div class="framework-card">
                <h4>📏 Why "Blocks" Structure?</h4>
                <ul>
                    <li>24 teams in clear competitive zones</li>
                    <li>Creates distinct strategic boundaries</li>
            ...
            <div class="framework-card">
                ...
                <ul>
                    ...
                    <li>Block 1 at Christmas: 75% finish top 2</li>
        ...
        <div class="pyramid-grid">
            ...
            <div class="league-block" style="border-left: 3px solid #5865F2;">
                ...
                <ul>
                    ...
                    <li>Block 3: Mid-Table (7-17)</li>
                    <li>Block 4: Danger Zone (18-21)</li>
                    <li>Block 5: Relegation (22-24)</li>
                </ul>
            </div>
            ...
            <div class="league-block" style="border-left: 3px solid #FFA500;">
                ...
                <ul>
                    ...
                    <li>Mid-table safety</li>
                    <li>Bottom 2: Relegated to Non-League</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="panel">
        ...
        <div class="insights-grid">
            <div class="insight-card" style="border-left: 3px solid #4CAF50;">
                ...
                <p class="big-stat">50 Points</p>
                <p>Teams reaching 50+ points in League One are virtually safe from relegation</p>
            </div>
            <div class="insight-card" style="border-left: 3px solid #FFD700;">
                <h4>🏆 Auto Promotion Target</h4>
                <p class="big-stat">88-95 Points</p>
                <p>Average points total for League One automatic promotion (top 2)</p>
            </div>
            <div class="insight-card" style="border-left: 3px solid #5865F2;">
                <h4>🎯 Playoff Spot</h4>
                <p class="big-stat">75-87 Points</p>
                <p>Typical range for securing a League One playoff place (3rd-6th)</p>
            </div>
            <div class="insight-card" style="border-left: 3px solid #F44336;">
                <h4>🔴 Relegation Zone</h4>
                <p class="big-stat">&lt;40 Points</p>
                <p>Teams finishing with less than 40 points face serious relegation danger</p>
            </div>
        </div>
    </div>
    ...
    <div class="panel">
        ...
        <div class="block-nav-grid">
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-1', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #FFD700; background: rgba(255, 215, 0, 0.05);">
                <span class="nav-emoji">🏆</span>
                <span class="nav-title">Block 1</span>
                <span class="nav-subtitle">Auto Promotion</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-2', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #5865F2; background: rgba(88, 101, 242, 0.05);">
                <span class="nav-emoji">🎯</span>
                <span class="nav-title">Block 2</span>
                <span class="nav-subtitle">Playoff Contenders</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-3', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #99AAB5; background: rgba(153, 170, 181, 0.05);">
                <span class="nav-emoji">⚖️</span>
                <span class="nav-title">Block 3</span>
                <span class="nav-subtitle">Mid-Table Security</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-4', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #FFA500; background: rgba(255, 165, 0, 0.05);">
                <span class="nav-emoji">⚠️</span>
                <span class="nav-title">Block 4</span>
                <span class="nav-subtitle">Relegation Battle</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-5', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #F44336; background: rgba(244, 67, 54, 0.05);">
                <span class="nav-emoji">🔴</span>
                <span class="nav-title">Block 5</span>
                <span class="nav-subtitle">Relegation Zone</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-dynamic', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #00D9FF; background: linear-gradient(135deg, rgba(0, 217, 255, 0.1), rgba(255, 107, 107, 0.1));">
                <span class="nav-emoji">🔥</span>
