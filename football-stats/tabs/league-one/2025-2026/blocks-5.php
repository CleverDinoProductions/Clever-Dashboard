...
/**
 * Block 5: Relegation Zone (Positions 22-24)
 * The Drop Zone - League Two Bound
 */
...
$blockInfo = [
    ...
    'color' => '#F44336',
    'positions' => '22-24',
    'description' => 'The Drop Zone - League Two Bound'
];
...
$teams = array_values(array_filter($tableView['standings'], function ($team) {
    return $team['position'] >= 22 && $team['position'] <= 24;
}));
...
<div class="block-detail-page">
    ...
    <div class="panel" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
        ...
        <?php football_stats_render_combined_table_controls($tableView, $currentMainTab ?? '2025-2026', 'league-one', $currentSubTab ?? 'blocks-5'); ?>
        <p style="margin-top: 15px; font-size: 1.1em; color: #ddd;">
            <?php echo $blockInfo['description']; ?> - Teams in the relegation zone facing the harsh reality of dropping to League Two.
        </p>
    ...
    <div class="panel">
        ...
        <div class="projections-grid">
            <?php foreach ($teams as $team):
                ...
                // Get points to 21st place (just above relegation zone)
                $team21st = null;
                ...
                $gapTo21st = $team21st ? ((int) $team21st['points'] - (int) $team['points']) : 0;
            ?>
            <div class="projection-card" style="border-left: 3px solid <?php echo $blockInfo['color']; ?>;">
                ...
                <div class="projection-stats">
                    ...
                    <div class="proj-row">
                        <span class="proj-label">Gap to 21st:</span>
                        <span class="proj-value" style="color: <?php echo $gapTo21st <= 3 ? '#FFA500' : '#F44336'; ?>;">
                            <?php echo $gapTo21st > 0 ? '-' . $gapTo21st : '+' . abs($gapTo21st); ?> pts
                        </span>
                    </div>
                    ...
                    <div class="proj-alert" style="background: rgba(244, 67, 54, 0.1); padding: 10px; border-radius: 5px; margin-top: 10px; border-left: 3px solid #F44336;">
                        <strong>🚨 PPG needed for escape (45pts):</strong> <?php echo $ppgNeededFor45; ?>
                        <br>
                        <span style="font-size: 0.85em; color: #888;">Must gain <?php echo $pointsNeededFor45; ?> points from <?php echo $remainingGames; ?> games</span>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
    ...
    <div class="panel">
        ...
        <div class="benchmarks-grid">
            <div class="benchmark-card" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
                ...
                <ul>
                    <li><strong>Current PPG:</strong> &lt; 0.9 (danger)</li>
                    <li><strong>Typical at MD23:</strong> &lt; 17 points</li>
                    <li><strong>Typical Season End:</strong> &lt; 35 points</li>
                    <li><strong>Win Rate:</strong> &lt; 28%</li>
                </ul>
            </div>
            <div class="benchmark-card" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
                <h4>💪 Great Escapes Needed</h4>
                <ul>
                    <li>Must win six-pointer matches</li>
                    <li>Home fortress mentality essential</li>
                    <li>Dramatic improvement required</li>
                    <li>January transfers can be decisive</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <!-- Historical Context -->
    <div class="panel">
        <h3>📜 Historical Relegation Data - League One</h3>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                        ...
                        <td style="padding: 10px; text-align: center;">&lt; 30 pts</td>
                        <td style="padding: 10px; text-align: center; color: #F44336;">95%+ Down</td>
                        <td style="padding: 10px; text-align: center; color: <?php echo $blockInfo['color']; ?>;"><strong>&lt; 0.79</strong></td>
                    </tr>
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                        ...
                        <td style="padding: 10px; text-align: center; color: <?php echo $blockInfo['color']; ?>;"><strong>0.79-0.92</strong></td>
                    </tr>
                    <tr>
                        <td style="padding: 10px;"><strong>Might Escape</strong></td>
                        <td style="padding: 10px; text-align: center;">36-39 pts</td>
        ...
        <div class="insight-box" style="background: rgba(244, 67, 54, 0.1); border-left: 3px solid <?php echo $blockInfo['color']; ?>; margin-top: 20px; padding: 15px; border-radius: 5px;">
            <h4>💡 Harsh Reality</h4>
            <p>
                Teams in Block 5 face an uphill battle. Only <strong>20% of teams in the League One relegation zone at Christmas survive</strong>.
                Reaching 40 points is the critical barrier. The financial cost of dropping to League Two is severe —
                clubs face significant budget cuts and must rebuild from a much lower commercial base.
                Every six-pointer match becomes a potential season-defining moment.
            </p>
        </div>
    </div>
    ...
    <div class="panel">
        <div class="block-navigation">
            ...
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'league-one', 'blocks-dynamic', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn" style="background: rgba(255, 215, 0, 0.1); color: #FFD700; border-color: #FFD700;">🔥 Live Data</a>
        </div>
    </div>
</div>
