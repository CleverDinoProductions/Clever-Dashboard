...
<div class="panel">
    ...
    <table>
        ...
        $total_played = $total_gr = $total_points = $total_ppg = $total_needed_38 = $total_needed_40 = $total_req = $total_ppg_req = $total_ppg_req_38 = $total_ppg_req_40 = $total_perf = $total_buffer = $total_max_pts = $total_max_pts_w = $total_max_pts_d = $total_max_pts_wd = 0;
        $num_teams = count($standings); // Number of teams for average calculations
        foreach ($standings as $team) {
            $total_played += ($team['played'] / 2); // Divide by 2 to avoid double counting since each game involves two teams
            $games_remaining = $total_games - ($team['played'] / 2); // Divide by 2 for same reason
            $total_gr += $games_remaining;
            $total_points += $team['points'];
            $ppg = ($team['played'] > 0) ? round($team['points'] / $team['played'], 2) : 0;
            $total_ppg += $ppg;
            $points_needed_38 = max(0, 38 - $team['points']);
            $points_needed_40 = max(0, 40 - $team['points']);
            $total_needed_38 += $points_needed_38;
            $total_needed_40 += $points_needed_40;
            $total_req += $points_needed_38; // You can change this to points_needed_40 or points_needed_max if you want to average those instead 
            $total_ppg_req += ($games_remaining > 0) ? round($points_needed_38 / $games_remaining, 2) : 0;
            $total_ppg_req_38 += ($games_remaining > 0) ? round($points_needed_38 / $games_remaining, 2) : 0;
            $total_ppg_req_40 += ($games_remaining > 0) ? round($points_needed_40 / $games_remaining, 2) : 0;
            $performance = ($team['played'] > 0) ? round($ppg * $games_remaining, 0) : 0;
            $total_perf += $performance;
            $buffer = $team['points'] - $team['played'];
            $total_buffer += $buffer;
            $max_points_possible = round($team['points'] + $performance, 0);
            $total_max_pts += $max_points_possible;
            $total_max_pts_w += 0; // Placeholder if you have max pts (W)
            $total_max_pts_d += 0; // Placeholder if you have max pts (D)
            $total_max_pts_wd += 0; // Placeholder if you have max pts (W/D)
        }
        ?>
        <tr style="background: #222; color: #FFCD00; font-weight: bold;">
            ...
            <td><?= $total_gr ?></td>
            <td><?= $total_points ?></td>
            <td><?= round($total_ppg, 2) ?></td>
            <td><?= $total_needed_38 ?></td>
            <td><?= $total_needed_40 ?></td>
            <td><?= $total_req ?></td>
            <td><?= $total_ppg_req ?></td>
            <td><?= $total_ppg_req_38 ?></td>
            <td><?= $total_ppg_req_40 ?></td>
            <td><?= $total_perf ?></td>
            <td><?= $total_buffer ?></td>
            <td><?= $total_max_pts ?></td>
            <td><?= $total_max_pts_w ?></td>
            <td><?= $total_max_pts_d ?></td>
            <td><?= $total_max_pts_wd ?></td>
        </tr>
        <tr style="background: #222; color: #00FF00; font-weight: bold;">
            <td colspan="4">Average</td>
            <td><?= round($total_points / $num_teams, 2) ?></td>
            <td><?= round($total_ppg / $num_teams, 2) ?></td>
            <td><?= round($total_needed_38 / $num_teams, 2) ?></td>
            <td><?= round($total_needed_40 / $num_teams, 2) ?></td>
            <td><?= round($total_req / $num_teams, 2) ?></td>
            <td><?= round($total_ppg_req / $num_teams, 2) ?></td>
            <td><?= round($total_ppg_req_38 / $num_teams, 2) ?></td>
            <td><?= round($total_ppg_req_40 / $num_teams, 2) ?></td>
            <td><?= round($total_perf / $num_teams, 2) ?></td>
            <td><?= round($total_buffer / $num_teams, 2) ?></td>
            <td><?= round($total_max_pts / $num_teams, 2) ?></td>
            <td><?= round($total_max_pts_w / $num_teams, 2) ?></td>
            <td><?= round($total_max_pts_d / $num_teams, 2) ?></td>
            <td><?= round($total_max_pts_wd / $num_teams, 2) ?></td>
        </tr>
    </table>
