<?php
/**
 * Blocks Overview - National League Blocks Framework
...
$blocks = [
    1 => ['teams' => [], 'range' => '1', 'title' => 'Auto Promotion', 'emoji' => '🏆', 'color' => '#FFD700', 'desc' => 'Automatic Promotion to League Two'],
    2 => ['teams' => [], 'range' => '2-7', 'title' => 'Playoff Contenders', 'emoji' => '🎯', 'color' => '#5865F2', 'desc' => 'Playoff spots for promotion to League Two'],
    3 => ['teams' => [], 'range' => '8-18', 'title' => 'Mid-Table Security', 'emoji' => '⚖️', 'color' => '#99AAB5', 'desc' => 'Safe & Stable'],
    4 => ['teams' => [], 'range' => '19-22', 'title' => 'Relegation Battle', 'emoji' => '⚠️', 'color' => '#FFA500', 'desc' => 'Danger Zone'],
...
<div class="blocks-overview-page">
    ...
    <div class="panel" style="border-left: 4px solid #5865F2;">
        <div class="overview-header">
            <h2>📊 Blocks of 5 Framework Overview</h2>
            <span class="season-badge">2025/26 Season</span>
        </div>
        <?php football_stats_render_combined_table_controls($tableView, $currentMainTab ?? '2025-2026', 'national-league', $currentSubTab ?? 'blocks-overview'); ?>
        <p style="margin-top: 15px; font-size: 1.1em; color: #ddd;">
            The National League is divided into 5 strategic blocks, each representing distinct competitive zones
    ...
    <div class="panel" style="border-left: 4px solid <?php echo $block['color']; ?>;">
        ...
        <div class="block-stats-summary">
            <?php
                ...
                $blockTargets = [
                    1 => ['target' => '90+ pts', 'ppg' => '2.2+', 'status' => 'Automatic promotion'],
                    2 => ['target' => '70-89 pts', 'ppg' => '1.7-2.1', 'status' => 'Playoff qualification'],
                    3 => ['target' => '48-69 pts', 'ppg' => '1.0-1.6', 'status' => 'Safe mid-table'],
        ...
        <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-' . $blockNum, $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="view-detail-btn" style="border-color: <?php echo $block['color']; ?>; color: <?php echo $block['color']; ?>;">
    ...
    <div class="panel">
        ...
        <div class="framework-grid">
            <div class="framework-card">
                <h4>📏 Why Blocks?</h4>
                <ul>
                    <li>24 teams across 5 strategic zones</li>
            ...
            <div class="framework-card">
                ...
                <ul>
                    ...
                    <li>Block 1 at Christmas: 75% promoted</li>
    ...
    <div class="panel">
        ...
        <div class="insights-grid">
            <div class="insight-card" style="border-left: 3px solid #4CAF50;">
                ...
                <p class="big-stat">48 Points</p>
                <p>Teams with 48+ points are generally safe from any relegation threat in the National League</p>
            </div>
            <div class="insight-card" style="border-left: 3px solid #FFD700;">
                <h4>🏆 Promotion Target</h4>
                <p class="big-stat">90+ Points</p>
                <p>Average points total for National League champions and automatic promotion spots</p>
            </div>
            <div class="insight-card" style="border-left: 3px solid #5865F2;">
                <h4>🎯 Playoff Spot</h4>
                <p class="big-stat">70-89 Points</p>
                <p>Typical range for playoff qualification (positions 2-7) in the National League</p>
            </div>
            <div class="insight-card" style="border-left: 3px solid #F44336;">
                <h4>🔴 Relegation Zone</h4>
                <p class="big-stat">&lt;38 Points</p>
                <p>Teams finishing with less than 38 points face serious relegation risk</p>
            </div>
        </div>
    </div>
    ...
    <div class="panel">
        ...
        <div class="block-nav-grid">
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-1', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #FFD700; background: rgba(255, 215, 0, 0.05);">
                <span class="nav-emoji">🏆</span>
                <span class="nav-title">Block 1</span>
                <span class="nav-subtitle">Auto Promotion</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-2', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #5865F2; background: rgba(88, 101, 242, 0.05);">
                <span class="nav-emoji">🎯</span>
                <span class="nav-title">Block 2</span>
                <span class="nav-subtitle">Playoff Contenders</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-3', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #99AAB5; background: rgba(153, 170, 181, 0.05);">
                <span class="nav-emoji">⚖️</span>
                <span class="nav-title">Block 3</span>
                <span class="nav-subtitle">Mid-Table Security</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-4', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #FFA500; background: rgba(255, 165, 0, 0.05);">
                <span class="nav-emoji">⚠️</span>
                <span class="nav-title">Block 4</span>
                <span class="nav-subtitle">Relegation Battle</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-5', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #F44336; background: rgba(244, 67, 54, 0.05);">
                <span class="nav-emoji">🔴</span>
                <span class="nav-title">Block 5</span>
                <span class="nav-subtitle">Relegation Zone</span>
            </a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-dynamic', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="block-nav-btn" style="border-left-color: #00D9FF; background: linear-gradient(135deg, rgba(0, 217, 255, 0.1), rgba(255, 107, 107, 0.1));">
                <span class="nav-emoji">🔥</span>
...
@media (max-width: 768px) {
    ...
    .framework-grid, .insights-grid { grid-template-columns: 1fr; }
