<?php
/**
 * Block 3: Mid-Table Security (Positions 8-18)
...
$teams = array_values(array_filter($tableView['standings'], function ($team) {
    return $team['position'] >= 8 && $team['position'] <= 18;
}));
...
<div class="block-detail-page">
    ...
    <div class="panel" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
        ...
        <?php football_stats_render_combined_table_controls($tableView, $currentMainTab ?? '2025-2026', 'national-league', $currentSubTab ?? 'blocks-3'); ?>
        <p style="margin-top: 15px; font-size: 1.1em; color: #ddd;">
            <?php echo $blockInfo['description']; ?> - Teams here are safe from relegation and too far from the top to challenge for promotion this season.
        </p>
    ...
    <div class="panel">
        ...
        <div class="projections-grid">
            <?php foreach ($teams as $team):
                ...
                $pointsNeededFor48 = max(0, 48 - $team['points']);
                $ppgNeededFor48 = $remainingGames > 0 ? round($pointsNeededFor48 / $remainingGames, 2) : 0;
                // Find 19th place for context
                $team19th = null;
                ...
                $gapTo19th = $team19th ? ((int) $team['points'] - (int) $team19th['points']) : 0;
            ?>
            <div class="projection-card" style="border-left: 3px solid <?php echo $blockInfo['color']; ?>;">
                ...
                <div class="projection-stats">
                    ...
                    <?php if ($gapTo19th >= 0): ?>
                    <div class="proj-row">
                        <span class="proj-label">Buffer above danger:</span>
                        <span class="proj-value" style="color: <?php echo $gapTo19th > 5 ? '#4CAF50' : '#FFA500'; ?>;">
                            +<?php echo $gapTo19th; ?> pts
                        </span>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="proj-row" style="border-top: 1px solid rgba(255,255,255,0.1); padding-top: 10px; margin-top: 10px;">
                        ...
                        <span class="proj-value" style="font-size: 1.2em; color: <?php echo $projectedPoints >= 48 ? $blockInfo['color'] : '#FFA500'; ?>;">
                            <strong><?php echo $projectedPoints; ?> pts</strong>
                        </span>
                    </div>
                    <div class="proj-row">
                        <span class="proj-label">Safety target:</span>
                        <span class="proj-value">~48 pts</span>
                    </div>
                    <div class="proj-row">
                        <span class="proj-label">Games remaining:</span>
                        <span class="proj-value"><?php echo $remainingGames; ?></span>
                    </div>
                    <?php if ($projectedPoints < 48): ?>
                    <div class="proj-alert" style="background: rgba(255, 152, 0, 0.1); padding: 10px; border-radius: 5px; margin-top: 10px; border-left: 3px solid #FF9800;">
                        <strong>⚠️ PPG needed for 48pts safety:</strong> <?php echo $ppgNeededFor48; ?>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="panel">
        ...
        <div class="benchmarks-grid">
            <div class="benchmark-card" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
                <h4>⚖️ Mid-Table Stability</h4>
                <ul>
                    <li><strong>Target PPG:</strong> 1.0-1.6</li>
                    <li><strong>Halfway (MD23):</strong> 24-34 points</li>
                    <li><strong>Season End (MD46):</strong> 48-69 points</li>
                    <li><strong>Win Rate:</strong> 35-50%</li>
                </ul>
            </div>
            <div class="benchmark-card" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
                <h4>📋 Season Objectives</h4>
                <ul>
                    <li>Secure 48+ points early</li>
                    <li>Develop young talent</li>
                    <li>Build for next season</li>
                    <li>Maintain fan engagement</li>
                </ul>
            </div>
            <div class="benchmark-card" style="border-left: 4px solid <?php echo $blockInfo['color']; ?>;">
                <h4>🔄 Block Dynamics</h4>
                <ul>
                    <li>11 teams competing for 11 spots</li>
                    <li>Good runs can reach playoffs</li>
                    <li>Bad runs can enter danger zone</li>
                    <li>Most movement happens here</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <!-- Historical Context -->
    <div class="panel">
        <h3>📜 Historical Block 3 Points</h3>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                        <td style="padding: 10px;"><strong>8th-10th Place</strong></td>
                        <td style="padding: 10px; text-align: center;">62-70 pts</td>
                        <td style="padding: 10px; text-align: center;">30-35 pts</td>
                        <td style="padding: 10px; text-align: center; color: <?php echo $blockInfo['color']; ?>;"><strong>1.4-1.6</strong></td>
                    </tr>
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                        <td style="padding: 10px;"><strong>11th-14th Place</strong></td>
                        <td style="padding: 10px; text-align: center;">55-63 pts</td>
                        <td style="padding: 10px; text-align: center;">26-32 pts</td>
                        <td style="padding: 10px; text-align: center; color: <?php echo $blockInfo['color']; ?>;"><strong>1.2-1.4</strong></td>
                    </tr>
                    <tr>
                        <td style="padding: 10px;"><strong>15th-18th Place</strong></td>
                        <td style="padding: 10px; text-align: center;">48-56 pts</td>
                        <td style="padding: 10px; text-align: center;">23-28 pts</td>
        ...
        <div class="insight-box" style="background: rgba(153, 170, 181, 0.1); border-left: 3px solid <?php echo $blockInfo['color']; ?>; margin-top: 20px; padding: 15px; border-radius: 5px;">
            <h4>💡 Key Insight</h4>
            <p>
                Block 3 is the most competitive area of the National League with 11 teams fighting for 11 spots.
                A strong run of 6+ wins can push a team into playoff contention, while 6+ defeats can drag them
                into the relegation battle. Consistency is the hallmark of successful Block 3 teams — avoiding
                long losing runs is as important as winning streaks.
            </p>
        </div>
    </div>
    ...
    <div class="panel">
        <div class="block-navigation">
            ...
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-dynamic', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn" style="background: rgba(153, 170, 181, 0.1); color: <?php echo $blockInfo['color']; ?>; border-color: <?php echo $blockInfo['color']; ?>;">🔥 Live Data</a>
            <a href="<?php echo htmlspecialchars(build_tab_url($currentMainTab ?? '2025-2026', $currentLeague ?? 'national-league', 'blocks-4', $tableViewNavParams), ENT_QUOTES, 'UTF-8'); ?>" class="nav-btn">Block 4 →</a>
        </div>
    </div>
</div>
